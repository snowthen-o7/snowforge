...
1. Paste your product feed CSV into a Google Sheet
2. Click **Feed Audit > Run Full Scan** (or a platform-specific audit) from the menu bar
3. Get a graded report with color-coded PASS, FAIL, and WARNING rows, plus a health score out of 100
...
### Required Fields
- Title present, length (25 to 150 chars), no promotional text (sale, discount, BOGO, free shipping)
- Description present, length (100 to 5000 chars), no raw HTML tags
- Price present, valid format, positive value
- Availability present with valid values
- Image link present and valid URL
- Product URL present
- Unique product IDs with duplicate detection
...
- Additional image coverage and duplicate detection

### Pricing
- Price outlier detection (flags values more than 10x or less than 0.1x the median)
- Sale price always less than regular price
- Sale price decimal precision, effective date validation

### Availability
- Out-of-stock percentage warning (flags if more than 50 percent OOS)
- Availability value distribution breakdown
- Preorder and backorder dates validation
...
- GTIN present, valid format (8, 12, 13, or 14 digits), check digit validation, restricted range detection
- Brand present, under 70 character limit
- MPN required when GTIN is missing
- Google product category mapped with granularity check
- Product type set with depth and format validation
...
- Expiration date format and past-date detection
- Unit pricing measure validation
- Identifier exists cross-check
- Condition matches title detection
- Multipack and is_bundle validation
- Age group required for apparel
- Ads redirect domain matching
- Promotion ID format
- Price not zero
- Availability uses Google-accepted values
...
The tool checks over 100 column name aliases across all platforms. You do not need to rename anything.

**Works with:**
- Shopify product CSV exports
- Google Merchant Center feed exports
- Meta and Facebook catalog exports
...
A companion guide covering:

- The 5 feed issues that cost the most money, with real-world patterns and revenue impact
